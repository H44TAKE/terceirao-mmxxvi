...
// --- COMPONENTES VISUAIS E ESTILOS GLOBAIS (FORA DO APP PARA EVITAR ERROS) ---
...
export default function App() {
  ...
  const handleAdminSubmit = (e: any) => {
    ...
    if (safeUser === 'melga' && safePass === '174700') {
      setIsAdmin(true);
      setShowAdminModal(false);
      setAdminForm({ user: '', pass: '' });
      setAdminError('');
    } else if (safeUser === 'bufus' && safePass === '2402') {
        setIsAdmin(true); setShowAdminModal(false); setAdminForm({ user: '', pass: '' }); setAdminError('');
    } else if (safeUser === 'uaijuana' && safePass === 'joanalinda') {
        setIsAdmin(true); setShowAdminModal(false); setAdminForm({ user: '', pass: '' }); setAdminError('');
    } else { 
  ...
  const updateAdminInstStatus = async (newStatus: string) => {
    ...
    try {
        ...
        if (newStatus === 'paid' && inst?.status !== 'paid') fireConfetti();
  ...
  const calculateDynamicProgress = () => {
    ...
    paidInstallments.forEach(inst => { if (monthData[inst.month]) currentTotalValue += monthData[inst.month].value; });
    ...
    return { 
        totalValue: currentTotalValue, maxGoal: MAX_GOAL, percentGeneral,
        activeGoalLabel: currentActiveGoal.label, activeGoalIcon: currentActiveGoal.icon,
        activeGoalTarget: currentActiveGoal.target, currentProgressInStep,
        percentTarget, missingForNextGoal: missingForNextGoal > 0 ? missingForNextGoal : 0
    };
  };
  ...
  const getGlobalStats = () => {
    ...
    return { paid, review, pending, paidPct: Math.round((paid/total)*100), reviewPct: Math.round((review/total)*100), pendingPct: Math.round((pending/total)*100) };
  };
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[999999] flex items-center justify-center p-4">
        <div className="bg-white/95 backdrop-blur-2xl w-full max-w-sm rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20 animate-in zoom-in duration-200 relative">
          <div className="p-6 text-center relative border-b border-gray-100">
            <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowAdminModal(false); }} className="absolute right-4 top-4 bg-gray-100 text-gray-500 hover:text-black rounded-full p-2 transition cursor-pointer z-[100]"><IconX size={16} /></button>
          ...
          <form onSubmit={handleAdminSubmit} className="p-6 space-y-4">
            {adminError && <div className="text-[#C41E1E] text-xs text-center bg-red-50 py-3 rounded-2xl font-bold uppercase">{adminError}</div>}
            ...
            <button type="submit" className="w-full bg-black text-white font-bold py-5 rounded-full hover:bg-[#C41E1E] transition shadow-lg uppercase tracking-widest text-xs cursor-pointer">Acessar Painel</button>
          </form>
        </div>
      </div>
    );
  ...
  const isProfileComplete = user && !user.isAnonymous && currentUserData;
  ...
  return (
    <div className="min-h-screen bg-[#F5F4EF] font-sans text-black pb-20 relative overflow-x-hidden">
      ...
      {/* CABEÇALHO COM CAMADAS CORRIGIDAS */}
      ...
      <main className="max-w-6xl mx-auto px-4 pt-28 relative z-10 w-full">
        {(!user || user.isAnonymous) && !isAdmin ? (
            <div className="flex flex-col items-center justify-center">
             <HeroArtwork />
             <div className="bg-white/90 backdrop-blur-xl p-8 w-full max-w-sm rounded-[2.5rem] ios-shadow border border-white/60 mt-4 text-center">
                <h2 className="text-2xl font-black text-black tracking-tighter mb-2 uppercase">Acesso do Formando</h2>
                <p className="text-gray-500 text-sm font-medium mb-6">Inicia sessão para conferir o teu carnê.</p>
                ...
                <button onClick={handleGoogleLogin} className="w-full bg-black text-white font-black py-4 rounded-full hover:bg-gray-800 transition active:scale-95 shadow-xl uppercase cursor-pointer">Entrar com Google</button>
             </div>
          </div>
        ) : (user && !user.isAnonymous && !currentUserData && !isAdmin) ? (
            <div className="flex flex-col items-center justify-center">
                <HeroArtwork />
                <div className="bg-white/90 backdrop-blur-xl p-8 rounded-[3rem] ios-shadow max-w-sm w-full relative border border-white/60 mt-4">
            ...
            <div className="space-y-6">
                ...
                {[1, 2, 3, 4, 5, 6, 7, 8].map((m) => {
                    ...
                    >
                        ...
                        {isPaid ? <IconCheckCircle2 className="text-[#C41E1E]" size={28} /> : isReview ? <IconClock className="text-amber-500" size={28} /> : !unlocked ? <IconLock className="text-gray-200" size={28} /> : <IconCircle className="text-gray-300" size={28} />}
                        </div>
                        <div className="mt-auto">
                        {isPaid ? <div className="w-full py-4 bg-[#C41E1E]/10 text-[#C41E1E] rounded-[1.2rem] text-center font-black uppercase text-[10px] tracking-widest">CONCLUÍDO</div> : 
                        isReview ? <div className="w-full py-4 bg-amber-50 text-amber-600 rounded-[1.2rem] text-center font-bold text-[10px] tracking-widest uppercase italic">Em Análise</div> : 
                        !unlocked ? <button disabled className="w-full py-4 bg-gray-100 text-gray-400 rounded-[1.2rem] text-center font-black text-[10px] tracking-widest flex items-center justify-center gap-2 cursor-not-allowed border border-gray-200 uppercase"><IconLock size={12} /> Bloqueado</button> : 
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="p-4 sm:p-6 pb-2">
                    ...
                    {['Todas as Turmas', '3º Ano A', '3º Ano B', '3º Ano Noturno'].map(t => (
                        <button key={t} onClick={() => setFilterClass(t)} className={`px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all active:scale-95 flex items-center gap-2 cursor-pointer ${filterClass === t ? 'bg-black text-white shadow-md' : 'bg-gray-50 text-gray-500 border border-gray-200 hover:bg-gray-100'}`}>
                ...
                <div className="divide-y divide-gray-50 px-2 sm:px-4">
                    ...
                    return (
                        ...
                        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
                            ...
                            {client.photoURL ? <img src={client.photoURL} alt="P" className="w-14 h-14 rounded-full border-2 border-white shadow-md" /> : <div className="w-14 h-14 bg-black text-white rounded-full flex items-center justify-center font-black text-xl border-4 border-white shadow-md">{client.name ? client.name.charAt(0).toUpperCase() : 'A'}</div>}
                            <div>
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            return (
                                <button key={m} onClick={() => setAdminManageInst({ client, month: m, inst })} className={`flex-1 min-w-[3.8rem] py-4 rounded-2xl flex flex-col items-center justify-center transition active:scale-90 shadow-sm cursor-pointer relative overflow-hidden ${isPaid ? 'bg-green-50 text-green-600 border border-green-100' : isReview ? 'bg-amber-100 text-amber-600 animate-pulse border border-amber-200 shadow-md ring-2 ring-amber-500/20' : 'bg-gray-50 text-gray-300 border border-gray-100 hover:bg-gray-200'}`}>
                                ...
                                {hasReceipt && <div className="absolute -top-1 -right-1 bg-amber-400 text-white p-1 rounded-bl-lg"><IconPaperclip size={10} /></div>}
                                </button>
                            );
                            })}
                        </div>
      ...
      {/* MODAL PAGAMENTO (ALUNO) - Z-INDEX 50 */}
      ...
      {/* MODAL GESTÃO PARCELA ADMIN - Z-INDEX 50 */}
                ...
                <div className="overflow-y-auto flex-1 pr-2">
                    <div className="mb-6 bg-[#F5F4EF] p-5 rounded-[2rem] border border-white shadow-inner">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => updateAdminInstStatus('pending')} className={`flex-1 py-3 rounded-2xl font-black text-[10px] uppercase transition cursor-pointer ${(!adminManageInst.inst || adminManageInst.inst?.status === 'pending') ? 'bg-black text-white shadow-md' : 'bg-white text-gray-400 border border-gray-200 hover:bg-gray-50'}`}>Pendente</button>
